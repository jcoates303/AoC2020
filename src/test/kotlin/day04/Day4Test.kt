...
class Day4Test {
    ...
    private var data = transformData(loadData("2020-Day4-Test.txt"))

    @Test
    fun `first record is correct`() {
        isValidPass1(data[0]) shouldBe true
    }

    @Test
    fun `second record is incorrect`() {
        isValidPass1(data[1]) shouldBe false
    }

    @Test
    fun `third record is correct`() {
        isValidPass1(data[2]) shouldBe true
    }

    @Test
    fun `fourth record is incorrect`() {
        isValidPass1(data[3]) shouldBe false
    }
